...
class DQDecoderLayer(MvPDecoderLayer):
    def __init__(self, space_size, space_center,
        ...
        # When2com style view selection (adaptive view weighting)
        self.view_query_proj = nn.Linear(d_model, 128)
        self.view_key_proj = nn.Linear(d_model, 128)
        # Communication gate (decides how much to use fused vs original features)
    ...
    def update_feature(self, tgt, attn_feature_views, query_pos=None):
        ...
        if method == 'when2com_gated':
            import math
            # 1. Content-aware Softmax view weighting (When2com inspired)
            q = self.view_query_proj(tgt)  # (batch, N, 128)
            feats = torch.stack(attn_feature_views, dim=2)  # (batch, N, n_views, 256)
            keys = self.view_key_proj(feats)  # (batch, N, n_views, 128)

            # Dot-product attention scores
            scores = torch.einsum('bqd,bqvd->bqv', q, keys) / math.sqrt(128)
            weights = F.softmax(scores, dim=-1)  # (batch, N, n_views)

            # Weighted fusion across views
            fused_feat = (weights.unsqueeze(-1) * feats).sum(dim=2)  # (batch, N, 256)

            # Pass through MLP
            tgt2 = self.feature_update_mlp(fused_feat)

            # 2. Communication Gate
            gate = self.comm_gate(tgt)  # (batch, N, 1)

            # Gated residual: gate controls how much fused info to use
            tgt_update = tgt + gate * self.dropout2(tgt2)
