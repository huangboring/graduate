...
from mvp_demo.timing import time_synchronized
...
class DyanmicQueryTransformer(MultiviewPosetransformer):
    ...
    def __init__(self, backbone, cfg):
        ...
        self.t_pose_origin = torch.load(t_pose_dir, map_location='cpu', weights_only=True)
    ...
    def initialize_reference_points(self, tgt, meta, method="gt_noise", value=None):
        ...
        # groundtruth poses.
        gt_ref_points = meta[0].get('joints_3d')
        # pred_result from voxelpose 
        vp_ref_points = meta[0].get('joints_3d_voxelpose_pred')
        ...
        if method == "gt_noise":
            if gt_ref_points is None:
                raise ValueError('gt_noise requires ground truth; use sample_space for live inference')
            ...
            x, y = torch.meshgrid(x_, x_, indexing='ij')
            ...
            noise_ref_points = ref_points_abs.expand(batch_size,-1,-1,-1).reshape(batch_size, -1, 3)
            noise_ref_points = noise_ref_points.float()
        
        elif method == "voxcel_pose_base":
            if vp_ref_points is None:
                raise ValueError('voxcel_pose_base requires external pose predictions')
    ...
    def extract_view_features(self, views):
        """Return feature levels in decoder order, stacked view-major then batch."""
        return self.backbone(torch.cat(views, dim=0), self.use_feat_level)[::-1]
    ...
    def forward(self, views=None, meta=None, output_dir='./', frame_id=None, threshold=0.5,
        ...
        if not views or meta is None or len(views) != len(meta):
            raise ValueError('views and camera metadata must have the same nonzero length')
        if precomputed_features is None:
            all_feats = self.extract_view_features(views)
        else:
            all_feats = precomputed_features
            expected = len(views) * views[0].shape[0]
            if not all_feats or any(x.ndim != 4 or x.shape[0] != expected for x in all_feats):
                raise ValueError('precomputed feature levels must match selected views and batch')
