...
def validate_3d(config, model, loader, output_dir, threshold, num_views=5, device="cpu", epoch=0, frame_id=None):
    ...
    with torch.no_grad():
        ...
        for i, (inputs, meta) in loader_all_frames:
            ...
            # Visualization
            if (i % max(1, len(loader) // 5) == 0):  # Save 5 samples per epoch
                try:
                    from utils.vis_per_view import visualize_multiview_2d
                    import cv2
                    
                    # Prepare images and camera params
                    images_list = []
                    camera_params_list = []
                    for v in range(num_views):
                        img_tensor = inputs[v][0] # Get first image in batch
                        ...
                        camera_params_list.append({
                            'K': meta[v]['camera']['K'][0].cpu().numpy(),
                            'R': meta[v]['camera']['R'][0].cpu().numpy(),
                            't': meta[v]['camera']['T'][0].cpu().numpy()
                        })
                    
                    # Filter predictions with score > threshold
                    pred_b0 = pred[0]
                    valid_preds = pred_b0[pred_b0[:, 0, 3] >= 0] # Filter by valid flag
                    pred_3d_poses = valid_preds[:, :, :3]
                    
                    out_path = os.path.join(output_dir, f'vis_2d_epoch{epoch}_batch{i}.png')
                    visualize_multiview_2d(images_list, pred_3d_poses, camera_params_list, out_path, gt_3d_poses=gt_3d[0].cpu().numpy())
